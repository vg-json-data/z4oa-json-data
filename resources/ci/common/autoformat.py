...
def autoformat(test=False):
    made_changes = False
    for path in [
        # *sorted(Path("./").glob("**/*.json")),             # universal
        *sorted(Path("./").glob("*.json")),             # global
        *sorted(Path("./region/").glob("**/*.json"))    # regions
    ]:
        with path.open("r") as room_file:
            room_json = json.load(room_file)
            # if room_json.get("$schema") not in \
            #     [
            #         "./schema/z3oa-helpers.schema.json",    # helpers
            #         "../../../schema/z3oa-room.schema.json" # regions
            #     ]:
            #     continue
