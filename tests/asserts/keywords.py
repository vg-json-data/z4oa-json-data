...
def process_keyvalue(k, v, metadata):
    ...
    badKeys = [
        ...
        "time",             # FIXME: to be validated by schema
    ...
    if kCheck in uniques and "twinDoorAddresses" not in k:
        if v not in uniques[kCheck]:
            ...
            isSkip = True
        elif kCheck == "nodeAddress" and v != "" and int(v, 16) in [0x189ca, 0x189d6]:
        ...
        if not isFloat and \
            ...
            # else:
                ...
                if not isArea and \
                    ...
                    goodValue = False
                    msg = f"🔴ERROR: {k} {v}"
                    msg = {"msg": msg, "region": metadata["region"] if "region" in metadata else ""}
                    if v == "":
                ...
                for room in [roomJSON]:
                            ...
                            if "leaveShinecharged" in strat["exitCondition"]:
                                if strat["exitCondition"]["leaveShinecharged"]["framesRemaining"] == "auto":
                                    if "entranceCondition" not in strat or "comeInShinecharged" not in strat["entranceCondition"]:
                                        ...
                                        messages["reds"].append(msg)
                                        messages["counts"]["reds"] += 1
                        if "nodeSubType" in node_lookup[toNode]:
                    ...
                    for node in room["nodes"]:
                        ...
                        if orphaned:
                            ...
                            if not foundNode:
                                if os.path.isfile(os.path.join(connectionPath, "inter.json")):
